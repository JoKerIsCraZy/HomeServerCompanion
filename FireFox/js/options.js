...
const showStatus = (service, msg, type) => {
  const el = document.getElementById(
    `status${service.charAt(0).toUpperCase() + service.slice(1)}`
  );
  el.textContent = msg;
...
const testConnection = async (service) => {
  ...
  const apiKey = document.getElementById(`${service}Key`)
  ...
  showStatus(service, `Testing connection to ${testUrl}...`, "success");
  ...
  try {
    ...
    if (service === "unraid" && apiKey) {
      ...
      options.body = JSON.stringify({
        query: "{ info { versions { core { unraid } } } }",
      }); // Simple query
    }
    ...
    showStatus(service, `Failed: ${err.message} \n(${testUrl})`, 'error');
    console.error(err);
  }
};
...
services.forEach((service) => {
  const saveBtn = document.getElementById(
    `save${service.charAt(0).toUpperCase() + service.slice(1)}`
  );
  const testBtn = document.getElementById(
    `test${service.charAt(0).toUpperCase() + service.slice(1)}`
  );
...
const renderOrderList = () => {
  chrome.storage.sync.get(["serviceOrder"], (items) => {
    ...
    currentOrder.forEach((service, index) => {
      ...
      row.style.cssText =
        "padding: 10px 15px; background: white; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center;";
